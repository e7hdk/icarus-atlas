...
## The Return of the Heraclidae (the House of Heracles complete)

The deferred final branch of the Heracles house — see **[docs/HERACLID_RETURN.md](HERACLID_RETURN.md)**. 14 figures (`tlepolemus`, `cleodaeus`, `aristomachus-heraclid`, `temenus`, `cresphontes`, `aristodemus`, `eurysthenes`, `procles`, `merope-messenia`, `aepytus-heraclid`, `polyphontes`, `oxylus`, `hyrnetho`, `deiphontes`) carry Heracles' line three generations past the hero into the Dorian conquest. From **Hyllus** (whose own Return failed when the Tegean `echemus` slew him) descend **Cleodaeus** and **Aristomachus**, whose three sons at last succeed: **Temenus** takes Argos, **Cresphontes** Messenia, and the twins of **Aristodemus** — **Eurysthenes** (Agiad) and **Procles** (Eurypontid) — found the dual Spartan kingship, while the Aetolian guide **Oxylus** wins Elis and the conquest drives out the existing Achaean `tisamenus` (son of Orestes). The Messenian strand gives **Merope**, the usurper **Polyphontes** who slays **Cresphontes**, and the avenging son **Aepytus** (suffixed `aepytus-heraclid` ≠ the Arcadian `aepytus-elatid`); the Argive strand **Hyrnetho** and her husband **Deiphontes**. **Tlepolemus**, a son of Heracles by Astyoche, slays the existing `licymnius` and flees to found the three cities of Rhodes (and dies at Troy by Sarpedon). Aristodemus' disputed death (Apollo's bolt vs slain vs lived) and the lot-casting for the three lands are kept as sourced prose. Homonyms suffixed (`aepytus-heraclid`, `merope-messenia`, `aristomachus-heraclid`). Each ships the three-layer codex. **With the Return, the House of Heracles is complete** — the Perseid bridge, the birth, the Twelve Labours, the death and apotheosis, and the Dorian return into history.

## The Children of the First Beings (the primordial broods complete)

The inner-cosmos completion — see **[docs/PRIMORDIAL_OFFSPRING.md](PRIMORDIAL_OFFSPRING.md)**. A 7-lens audit found whole canonical broods missing beside the ten primordials; 33 figures close them, in seven steps. **Batch 0** (edges only, no new nodes): re-attributed `python`→[hyginus,ovid], added `hyginus` to `typhon`, `apollodorus` to `cecrops`, `apollonius` to `cronus`, made all five Spartoi earth-born of Gaia, and gave `uranus` & `pontus` the Hyginus Aether+Dies parentage (disputes `uranus-parentage`/`pontus-parentage`); Gaia kept deliberately unbegotten. **Batch 1** — **Eurybia** (the fifth Pontid, consort of `crius`, mother of the orphaned `astraeus`/`pallas`/`perses`). **Batch 2** — the elder Uranus+Gaia broods: the three **Cyclopes** (`brontes-cyclops`, `steropes-cyclops`, `arges-cyclops`) and the three **Hecatoncheires** (`cottus`, `briareus`, `gyges-hecatoncheir`). **Batch 3** — the castration-blood brood: the **Erinyes**, **Gigantes** and **Meliae** (plural id `meliae`, distinct from the Oceanid `melia`). **Batch 4** — **Ourea**, **Tityos** and **Antaeus** (`antaeus-giant`, distinct from the Arcadian Argonaut). **Batch 5** — Nyx's remaining children: **Keres**, **Oneiroi**, **Hesperides**, **Philotes**, **Styx** (eldest Oceanid). **Batch 6** — the fifteen strife-children of **Eris** (`ponos`, `lethe-daimon` [distinct from the future river], `limos`, `algea`, `hysminai`, `makhai`, `phonoi`, `androktasiai`, `neikea`, `pseudea`, `logoi`, `amphillogiai`, `dysnomia`, `ate`, `horkos`). Homonyms suffixed per hard rule 7 (`-cyclops`, `-hecatoncheir`, `meliae`, `antaeus-giant`, `lethe-daimon`). 8 new documented contradictions (`pontus-parentage`, `hundred-handers-birth-order`, `erinyes-parentage`, `tityos-parentage`, `antaeus-parentage`, `hesperides-parentage`, `styx-parentage`, `ate-parentage`). All ship the three-layer codex; obscure personifications with no surviving figural art carry an honest empty Legacy gallery. **The inner cosmos is complete (499 stars).**

## The Divine Family (the Olympian retinue)

The gods' own household — see **[docs/DIVINE_FAMILY.md](DIVINE_FAMILY.md)**. 22 figures: the nine **Muses** (`calliope`, `clio`, `euterpe`, `erato-muse`, `melpomene`, `polyhymnia`, `terpsichore`, `thalia-muse`, `urania` — Zeus + Mnemosyne), the three **Charites/Graces** (`aglaea`, `euphrosyne`, `thalia-charis` — Zeus + Eurynome), the three **Horae/Seasons** (`eunomia`, `dike`, `eirene` — Zeus + Themis), **Hecate** (Perses + Asteria, `chthonic`), **Iris** (Thaumas + Electra), **Eileithyia** (Zeus + Hera), and the throne-guard children of Pallas & Styx (`nike`, `zelus`, `cratos`, `bia` — closing the Styx loose end from the primordial batch). Homonyms suffixed per hard rule 7: `erato-muse` ≠ the existing `erato-dryad`; the shared name Thalia splits into `thalia-muse` (Muse) and `thalia-charis` (Grace); the Oceanid mothers Eurynome & Electra stay in prose (no dangling edge). Cross-links: `aglaea` consort `hephaestus`, `iris` sibling `harpies` & ally `hera`, `eunomia` adversary `dysnomia`, the existing `sirens` gain their disputed Muse-mothers, `hyacinthus` gains the variant parent `clio`. Eight documented contradictions (`muse-parentage`, `charites-parentage`, `horae-roster`, `hecate-sphere`, `orpheus-parentage`, `rhesus-mother`, `sirens-mother`, `hephaestus-consort`). All ship the three-layer codex (63 verified artworks; `zelus` alone an honest empty gallery). **521 stars.**
