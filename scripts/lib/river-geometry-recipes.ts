...
export const RIVER_ANCHORS: Record<string, RiverAnchor> = {
  ...
  maeander: { point: [27.35, 37.55], maxDistKm: 40 },
  nilus: { point: [31.2, 30.0], maxDistKm: 80 },
  inachus: {
...
export const OSM_NAME_ALIASES: Record<string, string[]> = {
  inachus: ['Ινάχος', 'Inachos', 'Inachus'],
  ...
  hermus: ['Έρμος', 'Hermus', 'Gediz'],
  cayster: ['Κάυστρος', 'Cayster', 'Küçük Menderes'],
  'cephissus-boeotia': ['Κηφισός', 'Kifissos', 'Cephissus'],
  pleistos: ['Πλειστός', 'Pleistos', 'Pleistus'],
  strymon: ['Στρυμών', 'Strymon', 'Struma'],
  halys: ['Άλυς', 'Halys', 'Kızılırmak', 'Kizilirmak'],
  enipeus: ['Ενιπεύς', 'Enipeus', 'Enipefs'],
  'axios-macedon': ['Αξιός', 'Axios', 'Vardar'],
  sangarius: ['Σαγγάριος', 'Sangarius', 'Sakarya'],
  'ladon-arcadia': ['Λάδων', 'Ladon', 'Ladonas'],
  'asopus-phlias': ['Ασωπός', 'Asopos', 'Asopus'],
  orontes: ['Ορόντης', 'Orontes', 'Asi'],
  ister: ['Ἴστρος', 'Ister', 'Danube', 'Dunărea'],
  'styx-arcadia': ['Στύξ', 'Styx', 'Stiks'],
  euphrates: ['Ευφράτης', 'Euphrates', 'Firat'],
  tigris: ['Τίγρις', 'Tigris', 'Dicle'],
  jordan: ['Ιορδάνης', 'Jordan', 'Jordan River'],
  araxes: ['Ἀράξης', 'Araxes', 'Aras'],
  eridanus: ['Ηριδανός', 'Eridanus', 'Po', 'Padus'],
  acheron: ['Αχέρων', 'Acheron', 'Acheronas'],
  cocytus: ['Κώκυτος', 'Kokytos', 'Kokytos', 'Cocytus'],
};
...
export function buildFetchPlans(
  ...
  for (const feature of features) {
    ...
    const manualFile = FEATURE_OSM_CACHE[feature.id];
    if (manualFile && !Array.isArray(manualFile)) {
      const query = MANUAL_OSM_QUERIES[manualFile];
      if (query && !seenFiles.has(manualFile)) {
        seenFiles.add(manualFile);
        plans.push({ featureId: feature.id, cacheFile: manualFile, query, source: 'manual' });
      }
      continue;
    }
