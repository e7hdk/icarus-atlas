...
Beyond ancient sources, the project will eventually expand to include a "Wikipedia lens" for modern, synthesized encyclopedic summaries. Furthermore, the scope of an entity will grow to encompass its cultural legacy: related artworks, films, music, and historical artifacts.
...
See docs/PLAN.md for the milestone roadmap and decision log (chosen options are recorded there). Research catalogs: docs/SOURCES.md (7 author lenses), docs/CHARACTERS.md (live roster + data-entry conventions), docs/CONTRADICTIONS.md (verified inter-author disputes).
